...
Or even synthetic dataset with full response data.
...
In `inst/testdata` user may find a set of `qs` file that are example of gDR data model for different data type. In file `synthetic_list.yml` maybe be found list of these datasets.
...
In `inst/annotation_data` user may find `csv` files that are used in `gDRcore` for test of annotation functions.
